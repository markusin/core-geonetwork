...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="metadata-iso19139-2e">
		...
		<!-- Debug start 
		<xsl:message> Template metadata-iso19139-2e (.xsl)</xsl:message>
		Debug end
	...
	<xsl:template mode="iso19139" match="gmie:MIE_Metadata|*[@gco:isoType='gmie:MIE_Metadata']" priority="2">
		...
		<xsl:choose>
			...
			<xsl:when test="$currTab='acquisitionInfo'">
				<xsl:apply-templates mode="elementEP"
					select="gmi:acquisitionInfo|geonet:child[string(@name)='acquisitionInfo']">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
				</xsl:apply-templates>
			</xsl:when>
			...
			<xsl:when test="$currTab='ISOCore'">
				...
				<xsl:call-template name="displayGmieExtraElement">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
					<xsl:with-param name="dataset" select="$dataset"/>
				</xsl:call-template>
			...
			<!-- GMI Eurac tab 
			<xsl:when test="$currTab='gmieTabDesc'">
				<xsl:call-template name="gmieTabDesc">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
					<xsl:with-param name="dataset" select="$dataset"/>
				</xsl:call-template>
	...
	<xsl:template name="iso19139-2eSimple">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			...
			<xsl:with-param name="id"
				select="generate-id(/root/gui/strings/gmieTabDesc)"/>
			<xsl:with-param name="content">
				...
				<!-- license info -->
				<xsl:apply-templates mode="complexElement"
					select="gmd:identificationInfo/gmd:MD_DataIdentification/gmd:resourceConstraints">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="edit" select="$edit"/>
				</xsl:apply-templates>
	...
	<!-- ===================================================================
	<xsl:template name="displayGmieExtraElement">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
